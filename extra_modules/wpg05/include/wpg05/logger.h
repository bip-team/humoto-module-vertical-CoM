...
public:
  ...
  Logger(double timeStep, const StepPlan &stepPlan,
         const FootTraj &rightFootTraj, const FootTraj &leftFootTraj,
         const ProblemParameters &pbParams)
      : stepPlan_(stepPlan), rightFootTraj_(rightFootTraj),
        leftFootTraj_(leftFootTraj), timeStep_(timeStep), pbParams_(pbParams) {
  ...
  void addStateAndControl(const etools::Vector9 &state,
                          const etools::Vector3 &control) {
    addStateAndControl(state, control, pbParams_.zetaZero_);
  }
  void addStateAndControl(const etools::Vector9 &state,
                          const etools::Vector3 &control, const double &zeta) {
  ...
  Eigen::MatrixXd getPositionsAsMatrix() const {
    return toMatrix(positionsCoM_);
  }
  Eigen::MatrixXd getVelocitiesAsMatrix() const {
    return toMatrix(velocitiesCoM_);
  }
  Eigen::MatrixXd getAccelerationsAsMatrix() const {
    return toMatrix(accelerationsCoM_);
  }
  Eigen::MatrixXd getJerksAsMatrix() const { return toMatrix(jerksCoM_); }
  Eigen::MatrixXd getCoPMinsAsMatrix() const {
    return toMatrix(positionsCoPMin_);
  }
  Eigen::MatrixXd getCoPsAsMatrix() const { return toMatrix(positionsCoP_); }
  Eigen::MatrixXd getCoPMaxsAsMatrix() const {
    return toMatrix(positionsCoPMax_);
  }
  ...
  void logEverything(std::ofstream &logFile) const {
    ...
    logFile << "t = np.arange(0.," << (double)size() * (double)timeStep_ << ", "
            << timeStep_ << ")\n\n";
    ...
    logFile << "highestFeasibleZ = np.array("
            << highestFeasibleZ_.segment(0, positions.rows())
                   .transpose()
                   .format(cleanFmt)
            << ")\n";
    logFile << "xCoPMin = np.array("
            << positionsCoPMin.col(0).transpose().format(cleanFmt) << ")\n";
    logFile << "yCoPMin = np.array("
            << positionsCoPMin.col(1).transpose().format(cleanFmt) << ")\n";
    logFile << "zCoPMin = np.array("
            << positionsCoPMin.col(2).transpose().format(cleanFmt) << ")\n";
    logFile << "xCoP = np.array("
            << positionsCoP.col(0).transpose().format(cleanFmt) << ")\n";
    logFile << "yCoP = np.array("
            << positionsCoP.col(1).transpose().format(cleanFmt) << ")\n";
    logFile << "zCoP = np.array("
            << positionsCoP.col(2).transpose().format(cleanFmt) << ")\n";
    logFile << "xCoPMax = np.array("
            << positionsCoPMax.col(0).transpose().format(cleanFmt) << ")\n";
    logFile << "yCoPMax = np.array("
            << positionsCoPMax.col(1).transpose().format(cleanFmt) << ")\n";
    logFile << "zCoPMax = np.array("
            << positionsCoPMax.col(2).transpose().format(cleanFmt) << ")\n";
    ...
    logFile << "zMax = np.array(" << zMax_.transpose().format(cleanFmt)
            << ")\n";
    logFile << "dx = np.array("
            << velocities.col(0).transpose().format(cleanFmt) << ")\n";
    logFile << "dy = np.array("
            << velocities.col(1).transpose().format(cleanFmt) << ")\n";
    logFile << "dz = np.array("
            << velocities.col(2).transpose().format(cleanFmt) << ")\n";
    logFile << "ddx = np.array("
            << accelerations.col(0).transpose().format(cleanFmt) << ")\n";
    logFile << "ddy = np.array("
            << accelerations.col(1).transpose().format(cleanFmt) << ")\n";
    logFile << "ddz = np.array("
            << accelerations.col(2).transpose().format(cleanFmt) << ")\n";
    ...
    logFile << "time = np.array("
            << rightFootTraj_.t_.head(size()).transpose().format(cleanFmt)
            << ")\n";
  ...
  void plot3Dtraj(std::ofstream &logFile) const {
    ...
    logFile << "    y = pos[1]\n";
    logFile << "    z = pos[2]\n";
    logFile << "    pointsX = np.array([x-wX*0.5, x+wX*0.5, x+wX*0.5, "
               "x-wX*0.5, x-wX*0.5])\n";
    logFile << "    pointsY = np.array([y+wY*0.5, y+wY*0.5, y-wY*0.5, "
               "y-wY*0.5, y+wY*0.5])\n";
    ...
    logFile << "ax.plot(trajLFootX, trajLFootY, trajLFootZ, 'r', label='Left "
               "foot')\n";
    logFile << "ax.plot(trajRFootX, trajRFootY, trajRFootZ, 'b', label='Right "
               "foot')\n";
  ...
  void plotPlaneProjectionTraj(std::ofstream &logFile) const {
    ...
    logFile
        << "ax1.plot(y, highestFeasibleZ, '--g', label='z_Max = f(y_CoM)')\n";
    logFile << "ax2.plot(x, z, 'b', label='z_CoM = f(x_CoM)')\n";
    logFile
        << "ax2.plot(x, highestFeasibleZ, '--g', label='z_Max = f(x_CoM)')\n";
  ...
  /// @brief Writes a python file that can later be executed to display the
