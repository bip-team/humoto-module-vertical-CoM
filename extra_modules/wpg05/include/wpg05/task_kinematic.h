...
#include HUMOTO_CONFIG_DEFINE_ACCESSORS
  ...
  public:
    ...
    TaskKinematicsPolygon(const double gain = 1)
        : TaskAL("TaskKinematicsPolygon", gain) {}
              ...
              const humoto::ControlProblem &control_problem)
    {
      ...
      const humoto::wpg05::MPCVerticalMotion &mpc =
          dynamic_cast<const humoto::wpg05::MPCVerticalMotion &>(
              control_problem);
      ...
      for (long i = 0; i < N; ++i) {
        ...
        bBlocks_.segment(2 * i * cRows, cRows) =
            cstr.b - cstr.A * (com2rHip -
                               rFootTraj(mpc.currentStepIndex() + 1 + i) - s2a);
        // Left leg feasibility
        ABlocks_.block((2 * i + 1) * cRows, i * cCols, cRows, cCols) = cstr.A;
        bBlocks_.segment((2 * i + 1) * cRows, cRows) =
            cstr.b - cstr.A * (com2lHip -
                               lFootTraj(mpc.currentStepIndex() + 1 + i) - s2a);
        }
        ...
        b.noalias() =
            getGain() * (bBlocks_ - ABlocks_ * (posSelector * mpc.Ux() *
                                                mpc.currentState()));
    }

    static void
    computeAndLogHighestFeasibleZ(const humoto::wpg05::MPCVerticalMotion &mpc,
    ...
    static double
    computeHighestFeasibleZ(const humoto::wpg05::MPCVerticalMotion &mpc,
      ...
      rLeg.z() = std::sqrt(genCstr.radius_ * genCstr.radius_ -
                           rLeg.x() * rLeg.x() - rLeg.y() * rLeg.y());
      lLeg.z() = std::sqrt(genCstr.radius_ * genCstr.radius_ -
                           lLeg.x() * lLeg.x() - lLeg.y() * lLeg.y());
